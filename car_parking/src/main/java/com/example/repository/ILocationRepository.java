...
//    Location findLocation(@Param("id")Long id);
    ...
    @Query(value =
            "select f.id, f.name_floor,floor_id as floorId," +
            ...
            countQuery ="select f.id, f.name_floor,floor_id as floorId,section_id as sectionId,length,width,height from location join floor f on f.id = location.floor_id where f.name_floor like concat('%', :search ,'%')",nativeQuery = true )
